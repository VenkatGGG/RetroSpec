...
Async worker that consumes `analysis-jobs` from Redis and writes text-path session report cards back to the orchestrator.
...
- Supports provider modes:
  ...
  - `remote_text`: calls a single text-model HTTP endpoint and merges with heuristic output.
- Remote model responses are validated against a strict schema contract before being merged.
- Remote model payloads are sampled around marker windows and redacted for common sensitive tokens.
- Reports initial status as `pending` (needs visual verification) or `discarded` (low-confidence) via `POST /v1/internal/analysis-reports`.
...
- `ANALYZER_PROVIDER=heuristic|remote_text`
- `ANALYZER_TEXT_MODEL_ENDPOINT` (required for `remote_text`)
...
- `ANALYZER_DISCARD_UNCERTAIN=true|false` (when false, low-confidence sessions still proceed to visual stage)
